...
{
    /// <summary>
    ///     A class that finds types needed by Best by looping assemblies in the
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        private string _assemblySkipLoadingPattern =
            "^System|^mscorlib|^Microsoft|^CppCodeProvider|^VJSharpCodeProvider|^WebDev|^Castle|^Iesi" +
            "|^log4net|^NHibernate|^nunit|^TestDriven|^MbUnit|^Rhino|^QuickGraph|^TestFu|^Telerik" +
            "|^ComponentArt|^MvcContrib|^AjaxControlToolkit|^Antlr3|^Remotion|^Recaptcha|^AttributeRouting" +
            "|^Autofac|^Bootstrapper|^Calabonga|^DotNetOpenAuth|EntityFramework|^FluentSecurity|^Glimpse|" +
            "^ImageResizer|^Mvc|^PagedList|^Newtonsoft|^WebActivator|^WebMatrix|^AutoMapper|^FluentValidation" +
            "|^Mvc|^Org|^Mono|^nClam";
        ...
        /// <summary>
        ///     Gets or sets wether Best should iterate assemblies in the app domain when loading Best types. Loading patterns
        ...
        /// </summary>
        /// <remarks>
        ///     If you change this so that Best assemblies arn't investigated (e.g. by not including something like
        ///     "^Best|..." you may break core functionality.
        /// </remarks>
            ...
            bool onlyConcreteClasses = true)
        {
            ...
            try
            {
                foreach (Type t in from a in assemblies
                    from t in a.GetTypes()
                    where assignTypeFrom.IsAssignableFrom(t)
            ...
            {
                string msg = ex.LoaderExceptions
        ...
        {
            IEnumerable<Assembly> found = FindAssembliesWithAttribute<TAssemblyAttribute>();
        ...
        public IEnumerable<Assembly> FindAssembliesWithAttribute<T>(IEnumerable<Assembly> assemblies)
        {
            ...
            {
                List<Assembly> foundAssemblies = (from assembly in assemblies
            ...
            //We must do a ToList() here because it is required to be serializable
            //when using other app domains.
        ...
        {
            List<Assembly> assemblies = (from f in Directory.GetFiles(assemblyPath.FullName, "*.dll")
        ...
        /// <returns>A list of assemblies that should be loaded by the Best factory.</returns>
        ...
        /// </param>
        /// <returns>
        ///     True if the assembly should be loaded into Best.
        /// </returns>
