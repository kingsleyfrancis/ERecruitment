...
namespace ERecruitment.Core.IO
{
    public class FileSystemStorageProvider : IStorageProvider
    {
        ...
        {
            string mediaPath = AppDomain.CurrentDomain.GetData("DataDirectory").ToString();
            ...
            _publicPath = appPath + "App_Data/" + settings.DirectoryName + "/";
        }
        ...
        public IEnumerable<IStorageFolder> ListFolders(string path)
        {
            if (!Directory.Exists(Map(path)))
            {
                ...
                {
                    throw new ArgumentException(string.Format("The folder could not be created at path: {0}. {1}", path,
                        ex));
                }
            }
