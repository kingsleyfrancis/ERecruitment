...
namespace ERecruitment.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private Core.Identity.ApplicationSignInManager _signInManager;
        private Core.Identity.ApplicationUserManager _userManager;
        private readonly IAccountService _accountService;

        public AccountController(Core.Identity.ApplicationSignInManager signInManager,
        ...
        public Core.Identity.ApplicationUserManager UserManager
        {
            get { return _userManager; }
            private set { _userManager = value; }
        }

        public Core.Identity.ApplicationSignInManager SignInManager
        {
            get
            {
                return _signInManager ?? HttpContext.GetOwinContext()
                    .Get<Core.Identity.ApplicationSignInManager>();
            }
        ...
        public async Task<ActionResult> VerifyCode(VerifyCodeViewModel model)
        {
            ...
            var result =
                await
                    ...
                    model.Code, model.RememberMe, model.RememberBrowser);

            switch (result)
            {
                case SignInStatus.Success:
                    return RedirectToLocal(string.Empty, model.ReturnUrl);
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.Failure:
                default:
            ...
            string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        await SignInManager.SignInAsync(user, false, false);
                        return RedirectToLocal(info.Email, returnUrl);
                    }
                }
                AddErrors(result);
            }
        ...
        private ActionResult RedirectToLocal(string email, string returnUrl)
        {
            ...
            var userId = _accountService.GetClientId(email);

            var isAdmin = UserManager.IsInRole(userId, "admin");

            return isAdmin ? RedirectToAction("Index", "Dashboard",
                new {Area = "Admin"}) : 
